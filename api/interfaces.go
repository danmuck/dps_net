...
type NodeInterface interface {
	...
	FindValue(ctx context.Context, key NodeID) (value []byte, closest []ContactInterface, err error)

	// FindNode returns up to count closest peers to target.
	FindNode(ctx context.Context, key NodeID, count int) ([]ContactInterface, error)
...
type RoutingTableInterface interface {
	...
	GetBucket(i int) ([]ContactInterface, error)
...
type NetworkManagerInterface interface {
	...
	Ping(ctx context.Context, to ContactInterface) error
	// FindNodeRPC asks a peer for nodes closest to target.
	FindNodeRPC(ctx context.Context, to ContactInterface, target NodeID) ([]ContactInterface, error)
	// FindValueRPC asks a peer for the value or closest peers.
	FindValueRPC(ctx context.Context, to ContactInterface, key NodeID) (value []byte, closest []ContactInterface, err error)
	// StoreRPC tells a peer to store (key,value).
	StoreRPC(ctx context.Context, to ContactInterface, key NodeID, value []byte) error
}
