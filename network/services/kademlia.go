...
func NewKademliaService(rt *routing.RoutingTable) *KademliaServiceImpl {
	return &KademliaServiceImpl{RoutingTable: rt}
}
...
func (s *KademliaServiceImpl) Ping(ctx context.Context, req *PING) (*ACK, error) {
	...
	s.RoutingTable.Update(ctx, req.From)
	return &ACK{From: s.RoutingTable.GetLocal(), Value: req.Value}, nil
}
...
func (s *KademliaServiceImpl) FindNode(ctx context.Context, req *FIND_NODE) (*NODES, error) {
	s.RoutingTable.Update(ctx, req.From)
	nodes, err := s.RoutingTable.FindClosestK(ctx, api.SliceToNodeID(req.TargetId))
	if err != nil {
		return nil, err
	}
...
func (s *KademliaServiceImpl) FindValue(ctx context.Context, req *FIND_VALUE) (*VALUE, error) {
	...
	nodes, err := s.RoutingTable.FindClosestK(ctx, api.SliceToNodeID(req.Key))
	if err != nil {
		return nil, err
	}
